...
import com.amazonaws.serverless.sample.spring.model.LoginResponseVO;
...
public class CognitoHelper {
    ...
    public String validateUser(String username, String password) {
    	String response = "";
    	Gson gson = new GsonBuilder().setPrettyPrinting().disableHtmlEscaping().create();LoginResponseVO loginResponseVO =  new LoginResponseVO();
    	try {
    		init();
        	System.out.println("Pool Id: " + POOL_ID);
        	System.out.println("CLIENTAPP Id: " + CLIENTAPP_ID);
        	AuthenticationHelper helper = new AuthenticationHelper(POOL_ID, CLIENTAPP_ID, Constants.REDIRECT_URL);
            response = helper.performSRPAuthentication(username, password);
            
            if ( response != null &&  !response.equals("")) {
            	loginResponseVO.setStatus("success");
            	loginResponseVO.setMessage("User Authenticated Successfully");
            	loginResponseVO.setToken(response);
            } else {
            	loginResponseVO.setStatus("failed");
            	loginResponseVO.setMessage("User Authentication Failed");
            	loginResponseVO.setToken("");
            }
    	} catch (Exception e) {
